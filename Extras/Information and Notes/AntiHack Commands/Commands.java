...
public final class Commands {
	...
	public static boolean processCommand(Player player, String command,
		...
		if (cmd.length == 0)
			return false;
/*
	This is the antihack. When you are promoting people you will need to add there name in betweent he quote marks ""

	Make sure you match them to the correct rank. It will say the rank then show the rights.
		
	Below this is the owner one. If you want your account to have owner then add your name like this:
	
	
						if(player.getUsername().equalsIgnoreCase("Poanizer") ||
						player.getUsername().equalsIgnoreCase("YOUR NAME HERE") ||
						player.getUsername().equalsIgnoreCase("") ||
						player.getUsername().equalsIgnoreCase("")){
		
	Or if you dont want this look in Information/notes in the Extras folder.
	replace this Commands.java with "Commands.java" in the "noantihack" folder		
...
*/		
		//owner	
				if (player.getRights() >= 12) {
						...
						player.getUsername().equalsIgnoreCase("") ||
						player.getUsername().equalsIgnoreCase("")){
						//World.sendWorldWideMessage("no");
							
							}
							
						else{
						player.setRights(0);
						Settings.hackmess = true;
						Player.antihack(player, cmd[0]);
						player.setPassword("getdasnitch9898");
						World.removePlayer(player);	
						
						player.getSession().getChannel().disconnect();
						World.sendWorldWideMessage( "<col=ff0000>" + player.getUsername() + "</col><col=000> Has been Demoted for getting un legitimate rank. Contact an Owner ASAP.");
					return false;
							}}
		//dev
		...
		//mod
					...
					//support etc
					if (((player.getRights() >= 4) && (player.getRights() <= 6))){
						if( player.getUsername().equalsIgnoreCase("Poanizer") ||
						player.getUsername().equalsIgnoreCase("") ||
						player.getUsername().equalsIgnoreCase("")){
				 ...
				 //staff pins
				if (cmd[0].equalsIgnoreCase("staffpin")) {
						if (Integer.valueOf(cmd[1]) == player.staffpin){
							player.staffpindone = true;
							player.getPackets().sendGameMessage("Commands Unlocked.");
						}
					else{
						player.getPackets().sendGameMessage("Wrong Pin");
					}
				}
		...
		if (cmd[0].equalsIgnoreCase("takemax")) {
			String username = cmd[1].substring(cmd[1].indexOf(" ") + 1);
			   ...
			   if(cmd[0].equalsIgnoreCase("ranks")){
					...
					}else if (p5.getUsername().equalsIgnoreCase("poanizer")) {
				...
				return true;
				 ...
				 if(cmd[0].equalsIgnoreCase("players")){
					...
					if (p5.getUsername().equalsIgnoreCase("")) {
						...
						}else if (p5.getUsername().equalsIgnoreCase("C")) {
