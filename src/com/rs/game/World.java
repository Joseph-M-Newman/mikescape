...
public static boolean BoxInUse;
...
public static boolean QBD;
	...
	//	}
		...
		n = new TzTokJad(id, tile, mapAreaNameHash,
			...
			n = new Level1(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea,
					spawned);
		else if (id == 12218)
			n = new Level2(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea,
					spawned);
		else if (id == 12222)
			n = new Level3(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea,
					spawned);
		else if (id == 12226)
			n = new Level4(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea,
					spawned);
		else if (id == 15581)
			n = new PartyDemon(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea,
					spawned);
			...
			public void run() {
				try {
					for (Player player : World.getPlayers()) {
						...
						SerializableFilesManager.updatingBackup(String.valueOf(player.getUsername()));
						player.realFinish();
					}
					IPBanL.save();
					PkRank.save();
...
public static final void addGroundItem(final Item item,
	final WorldTile tile, final Player owner,
      final boolean underGrave, final long hiddenTime/* default 3minutes */,
      boolean invisible) {
	  
     if (owner != null && owner.getBoneDelay() != -1
       && owner.getBoneDelay() > Utils.currentTimeMillis())
      return;
     final FloorItem floorItem = new FloorItem(item, tile, owner,
       owner != null ? underGrave : false, invisible);
     final Region region = getRegion(tile.getRegionId());
     region.forceGetFloorItems().add(floorItem);
     if (invisible && hiddenTime != -1L) {
      if (owner != null)
      ...
      WorldTasksManager.schedule(new WorldTask() {
       ...
       public void run() {
		...
		//atempt to not make non tradables dropable
	//works but the item cannot be picked up
	int itemIds = floorItem.getId();
String itemName = item.getDefinitions() == null ? "" : item.getDefinitions().getName().toLowerCase();
	if(stringContainsItemFromList(itemName, Settings.DONATOR_ITEMS)){
			 for (Player player : World.getPlayers()) {
           if (player != null
             && player != owner
             && player.hasStarted()
	...
	else if(stringContainsItemFromList(Integer.toString(itemIds),Settings.NON_TRADABLE)){
			 for (Player player : World.getPlayers()) {
          //nothing
          }
		}
       ...
       /*
        * { region = region1; floorItem = flooritem; tile = worldtile;
        * owner = player; underGrave = flag; super(); }
        */
      }, (int) hiddenTime);
     return;
     }
	...
	public static final boolean isMultiArea(WorldTile tile) {
		...
		return (destX >= 3200 && destX <= 3390 && destY >= 3840 && destY <= 3967) // wild
				...
				|| (destX >= 3415 && destX <= 3430 && destY >= 3155 && destY <= 3172)//Party
				...
				|| (destX >= 3789 && destY >= 2849 && destX <= 3793 && destY <= 2855) // HOME pvp
				
				|| (destX >= 2840 && destX <= 2950 && destY >= 5190 && destY <= 5230) // godwars
				|| (destX >= 3547 && destX <= 3555 && destY >= 9690 && destY <= 9699) // zaros
				/** revs98987*/ /** || (destX >= 3116 && destX <= 3118 && destY >= 10116 && destY <= 10119) */
				|| (destX >= 3093 && destX <= 3120 && destY >= 3915 && destY <= 3950)
				
				//safe pvp
				|| (destX >= 2605 && destX <= 2596 && destY >= 3153 && destY <= 3160)
				
				|| (destX >= 2605 && destX <= 2596 && destY >= 3163 && destY <= 3167)
				
				|| (destX >= 2598 && destX <= 2596 && destY >= 3162 && destY <= 3161)
				
				|| (destX >= 2600 && destX <= 2604 && destY >= 3162 && destY <= 3161)
				//dks
				|| (destX >= 2889 && destX <= 2939 && destY >= 4472 && destY <= 4424)
				//potest2
				
				|| (destX >= 3214 && destX <= 3237 && destY >= 2832 && destY <= 2851) //kk
				
				//new arena test
				//|| (destX >= 2441 && destX <= 2482 && destY >= 4237 && destY <= 4283)
				...
				//|| (destX >= 3214 && destY >= 2851 && destX <= 2832 && destY <= 2832) //KK
				
				|| (destX >= 2756 && destY >= 5539 && destX <= 2875 && destY <= 5629) //clanwars danger
				|| (destX >= 2901 && destX <= 2922 && destY >= 4439 && destY <= 4461) // Dagannoth Kings																		
				
				
				|| (destX >= 3089 && destX <= 3086 && destY >= 3498 && destY <= 3494) // safe pvp																		
				
				|| (destX >=2185  && destX <= 2195 && destY >= 3302  && destY <= 3315 ) // po																	
				
				|| (destX >= 3011 && destY >= 10052 && destX <= 3132 && destY <= 10175); //revs
