...
public class PlayerCombat extends Action {
	...
	public int mageAttack(Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			/*case 47:// crumble undead
					...
					player.setNextAnimation(new Animation(10184));
					target.setNextAnimation(new Animation(16380));
					delayMagicHit(5,getMagicHit(player, getRandomMagicMaxHit(player, 500)));
					mage_hit_gfx = 574;
					base_mage_xp = 100;
					target.addFreezeDelay(10000, true);
					player.addFreezeDelay(15000, true);
					//World.sendProjectile(player, target, 178, 18, 18, 50, 50, 0, 0);
			...
			case 66:// Saradomin Strike
				...
				base_mage_xp = 34.5;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 350)));
				return 5;
			...
			case 91:// fire surge
				...
				mage_hit_gfx = 2741;
				base_mage_xp = 80;
				damage = getRandomMagicMaxHit(player, 330);
				if (target instanceof NPC) {
					NPC n = (NPC) target;
					if (n.getId() == 9462) // ice verm
						damage *= 3;
				}
				delayMagicHit(2, getMagicHit(player, damage));
			...
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				int hit = getRandomMagicMaxHit(player, 200 + boost);
				if (hit != 0 && hit < boost)
					hit += boost;
			...
			case 26:// Ice Burst
				...
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 160)));
				World.sendProjectile(player, target, 360, 18, 18, 50, 50, 0, 0);
				return 4;
			case 29:// Smoke Blitz
				player.setNextAnimation(new Animation(1978));
				mage_hit_gfx = 387;
			...
			case 25:// Blood Blitz
				...
				blood_spell = true;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 270)));
				World.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
				return 4;
			...
			case 21:// Ice Blitz
				...
				blood_spell = true;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 310)));
				World.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
				return 4;
			...
			case 23: // ice barrage
				...
				base_mage_xp = 52;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 320)));
						World.sendProjectile(player, target, 362, 18, 18, 50, 50, 0, 0);
				return 4;
				...
				freeze_time = 2000;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 352)));
				World.sendProjectile(player, target, 1853, 18, 18, 50, 50, 0, 0);
				return 4;
			}
			}
			else if (player.getCombatDefinitions().getSpellBook() == 430) {//lunar
			switch (spellId) {
				case 52://heal other
				
					player.setNextAnimation(new Animation(10753));
					player.setNextGraphics(new Graphics(2345));
					target.heal(player.getHitpoints() / 2);
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			if (weaponId != -1) {
				...
				} else if (weaponId == 24474) {// handcannon
					player.setNextAnimation(new Animation(getWeaponAttackEmote(
							weaponId, attackStyle)));
				...
				} else if (weaponId == 6082) {// bazooka
					
						player.setNextGraphics(new Graphics(2936));
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 11694: // ags
				...
				case 23679: //lucky ags v2
				player.setNextAnimation(new Animation(710));
				player.setNextGraphics(new Graphics(2407));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 23679: //lucky ags
				player.setNextAnimation(new Animation(14307));
				player.setNextGraphics(new Graphics(2323));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
				...
				break;*/
			case 20821: // 20th
				player.setNextAnimation(new Animation(437));
				player.setNextGraphics(new Graphics(988));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
								player,
								getRandomMaxHit(player, weaponId, attackStyle,
										false, true, 1.80, true)));
				break;
			case 24187: // 10th
				player.setNextAnimation(new Animation(12662));
				player.setNextGraphics(new Graphics(129));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 13901:
				...
				break;
			case 24909:  //Leg Sword
				player.setNextAnimation(new Animation(2890));
				player.setNextGraphics(new Graphics(435));
			...
			case 15438: // staff
				...
				player.setNextGraphics(new Graphics(128));
				int hit5 = getRandomMaxHit(player, weaponId, attackStyle,
						false, true, 1.0, true);
				int hit6 = hit5 == 0 ? getRandomMaxHit(player, weaponId,
						attackStyle, false, true, 1.0, true) : hit5;
				if (hit5 == 0 && hit6 == 0) {
					...
					if (hit3 == 0) {
						...
						if (hit4 == 0) {
							delayNormalHit(weaponId, attackStyle,
									getMeleeHit(player, hit5),
									getMeleeHit(player, hit6));
							...
							delayHit(1, weaponId, attackStyle,
									getMeleeHit(player, hit3),
									getMeleeHit(player, (int) (hit4 * 1.8)));
						}
					} else {
						delayNormalHit(weaponId, attackStyle,
								getMeleeHit(player, hit5),
					...
					delayNormalHit(weaponId, attackStyle,
							getMeleeHit(player, hit5),
							getMeleeHit(player, hit5 == 0 ? hit6 : hit6 / 2));
					delayHit(
							1,
							weaponId,
							attackStyle,
							getMeleeHit(player, hit5 == 0 ? hit6 / 2 : hit6 / 4),
							getMeleeHit(player, hit6 / 4));
				}
				break;
			case 14484: // d claws
		...
		//int weaponId = player.getEquipment().getWeaponId();
		int gloveIds = player.getEquipment().getGloveId();
		if (player.getEquipment().getWeaponId() == -1){
		switch (gloveIds) {

				case 22361:// goliath
				player.setNextAnimation(new Animation(14393));
				//player.setNextGraphics(new Graphics(340, 0, 96 << 16));
				delayNormalHit(
						weaponId,
						attackStyle,
		...
		switch (weaponId) {
			case 20929:// katana
				...
				case 24875:// poanizer maul
				player.setNextAnimation(new Animation(13055));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 24864:// masa
				player.setNextAnimation(new Animation(12033));
				player.setNextGraphics(new Graphics(2688, 0, 96 << 16));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
				...
				case 24881:// king dick
				player.setNextAnimation(new Animation(10499));
				player.setNextGraphics(new Graphics(1835, 0, 96 << 16));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle,
		...
		if (target instanceof NPC) {
			NPC n = (NPC) target;
			if (n.getId() == 8133 && hasZSpear(player))
				hit += 70;
		}
	...
	public boolean hasZSpear(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
