...
public class Slayer {
	...
	public enum Master {
		...
		SPRIA(8462, "Travel north of Taverly to get here; Burthorpe.", 3, 1,
				new int[] { 15, 50 }, "Bird", "Chicken", "Rat", "Bear",
				"Bat", "Cow", "Cow calf", "Crawling hand", "Cave slime",
				"Banshee", "Dwarf", "Ghost", "Goblin", "Spider",
				"Monkey", "Scorpion"),
	...
	public static void killedTask(Player player, NPC monster) {
		SlayerTask task = (SlayerTask) player.getTemporaryAttributtes().get(
				"SlayerTask");
			if (monster.getName() == ("SlayerTask")){
		task.setAmount(task.getAmount() - 1);
		player.getSkills().addXp(Skills.SLAYER,
				monster.getCombatDefinitions().getHitpoints() / 10);
				}
		if (task.getAmount() == 0) {
			task = null;
			player.slayerPoints += (int) monster.getCombatDefinitions().getHitpoints() / 10;
			player.getTemporaryAttributtes().put("SlayerTask", task);
		}
	}
