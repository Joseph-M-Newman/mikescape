...
public class SlayerTask implements Serializable {
	...
	//add more complex system
		player.getSkills().addXp(Skills.SLAYER,  n.getCombatDefinitions().getHitpoints() /100);
		monstersLeft--;
		...
		if (monstersLeft < 1) {
			...
			if (n.getCombatLevel() >=500){
			player.SlayerPoints += 6;
			player.getPackets().sendGameMessage("You have gained 6 slayer points. You now have " + player.SlayerPoints + " Slayer Points");

			}
			else if (n.getCombatLevel() >=50 ){
			player.SlayerPoints += 2;
			player.getPackets().sendGameMessage("You have gained 2 slayer point. You now have " + player.SlayerPoints + " Slayer Points");

			}
			else{
			player.SlayerPoints += 1;
			player.getPackets().sendGameMessage("You have gained a slayer point. You now have " + player.SlayerPoints + " Slayer Points");
