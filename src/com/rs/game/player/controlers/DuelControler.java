		...
		if (target.getTemporaryAttributtes().get("DuelChallenged") == player) {
			...
			target.setDuelConfigurations(duelConfigurations);
			 for(int slot = 0; slot <= 13; slot++) {
				target.getEquipment().refresh(slot);
				player.getEquipment().refresh(slot);
				
				//target.setNextForceTalk(new ForceTalk(String.valueOf(slot) + "controler"));
				//player.setNextForceTalk(new ForceTalk(String.valueOf(slot) + "controler"));
