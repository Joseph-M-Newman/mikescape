...
public class Reply extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		 ...
		 if (stage == -1) {
			...
			sendEntityDialogue(SEND_1_TEXT_CHAT,
								new String[] { NPCDefinitions.getNPCDefinitions(npcId).name, "Oh yea na, Not to bad either" },
			IS_NPC, npcId, 9827);
			stage = 2;
		} else if (stage == 2) {
			sendEntityDialogue(SEND_1_TEXT_CHAT,
			...
			sendEntityDialogue(SEND_2_TEXT_CHAT,
								new String[] { NPCDefinitions.getNPCDefinitions(npcId).name, "Man you would believe it!", "If only more people like yourself talked to me." },
			IS_NPC, npcId, 9827);
			stage = 4;
		} else if (stage == 4) {
			sendEntityDialogue(SEND_2_TEXT_CHAT,
