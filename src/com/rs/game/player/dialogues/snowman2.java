...
public class snowman2 extends Dialogue {
	...
	public void start() {
		...
		sendEntityDialogue(SEND_1_TEXT_CHAT,
				new String[] { NPCDefinitions.getNPCDefinitions(npcId).name,
						"This is the skilling event!"}, IS_NPC, npcId, 9785);}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (stage == -1) {			
			...
			sendEntityDialogue(SEND_1_TEXT_CHAT,
					new String[] { NPCDefinitions.getNPCDefinitions(npcId).name, "train the various skills and make the items."}, IS_NPC, npcId, 9827);
			stage = 2;
		} else if (stage == 2) {
			sendEntityDialogue(SEND_1_TEXT_CHAT,
								new String[] { player.getDisplayName(), "You can then sell them to the shop for rare items." },
			IS_NPC, npcId, 9827);
			stage = 4;
