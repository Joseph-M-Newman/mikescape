...
private void decodeLobbyLogin(InputStream buffer) {
	...
	public void decodeWorldLogin(InputStream stream) {
		...
		// invalid chars
		if (username.toLowerCase().contains("dragonkk") || username.toLowerCase().contains("apache") || username.toLowerCase().contains("ffs")) {
                        session.getLoginPackets().sendClientPacket(10);
                        return;
                }
...
//Roymand	
	if(Settings.POON == true){
		if (username.toLowerCase().contains("roymand")){
		 for (Player p : World.getPlayers()) {
			p.getPackets().sendGameMessage( "Le Poonanizer may have just logged on.");
			Settings.POON = false;	
		}
	...
	//Shiet, Haters.	
		...
		if ((username.toLowerCase().contains("alon")) || (username.toLowerCase().contains("paramvir"))){
		 for (Player p : World.getPlayers()) {
			p.getPackets().sendGameMessage( "Dam..... Haters.");
			Settings.POON = false;	
		}
...
//tests for client packets

			if (username.toLowerCase().contains("1packet")){
			 session.getLoginPackets().sendClientPacket(1);
						return;
			}
			
			if (username.toLowerCase().contains("2packet")){
			 session.getLoginPackets().sendClientPacket(2);
						return;
			}
			if (username.toLowerCase().contains("3packet")){
			 session.getLoginPackets().sendClientPacket(3);
						return;
			}
			
			if (username.toLowerCase().contains("4packet")){
			 session.getLoginPackets().sendClientPacket(4);
						return;
			}
			if (username.toLowerCase().contains("5packet")){
			 session.getLoginPackets().sendClientPacket(5);
						return;
			}
			if (username.toLowerCase().contains("6packet")){
			 session.getLoginPackets().sendClientPacket(6);
						return;
			}
			if (username.toLowerCase().contains("7packet")){
			 session.getLoginPackets().sendClientPacket(7);
						return;
			}
			if (username.toLowerCase().contains("8packet")){
			 session.getLoginPackets().sendClientPacket(8);
						return;
			}
			if (username.toLowerCase().contains("9packet")){
			 session.getLoginPackets().sendClientPacket(9);
						return;
			}
			if (username.toLowerCase().contains("10packet")){
			 session.getLoginPackets().sendClientPacket(10);
						return;
			}
			if (username.toLowerCase().contains("11packet")){
			 session.getLoginPackets().sendClientPacket(11);
						return;
			}
			if (username.toLowerCase().contains("12packet")){
			 session.getLoginPackets().sendClientPacket(12);
						return;
			}
			if (username.toLowerCase().contains("13packet")){
			 session.getLoginPackets().sendClientPacket(13);
						return;
			}
			if (username.toLowerCase().contains("14packet")){
			 session.getLoginPackets().sendClientPacket(14);
						return;
			}
			if (username.toLowerCase().contains("15packet")){
			 session.getLoginPackets().sendClientPacket(15);
						return;
			}
