	...
	public void processPackets(final int packetId, InputStream stream,
		...
		if (packetId == PING_PACKET) {
			...
			if (player.getMuted() > Utils.currentTimeMillis()) {
				player.getPackets().sendGameMessage(
						"You temporary muted. Try again Later.");
				return;
			}
			...
			if (player.getLastPublicMessage() > Utils.currentTimeMillis())
				return;
			if (player.getMessageAmount() > 7) {
				if(player.mutewarn == true){
				player.setMuted(Utils.currentTimeMillis() + (2 * 60000 * 60 * 1000));//sets mute for 3 hours
				player.getPackets().sendGameMessage( "You have been muted for spamming after being warned you are muted for 3 hours.");
				player.mutewarn = false;
			}
				else{
				player.mutewarn = true;
				player.setMuted(Utils.currentTimeMillis() + (2 * 60000));//sets mute for 2 mins
				player.getPackets().sendGameMessage( "You have been muted for spamming for 2 minutes dont spam again.");
			}
			}
			...
			//Chatlog. will log everything typed.
			...
			if(message.contains("0hdr2ufufl9ljlzlyla") || message.contains("0hdr")) 
                                return;
			//logs the message that contains any keywords. Handy to give a brief check on server.			
			if(message.contains("dupe") || message.contains("bug") || message.contains("glitch") || message.contains("lure") || message.contains("scam") || message.contains("ddos") || message.contains(" ip ") || message.contains("abuse") || message.contains("leech")) {
								Player.keyWordLog(player, message);
			...
			if (chatType == 1){
				if(player.getcustomTitle() == null){
					...
					player.sendFriendsChannelMessage(player.getcustomTitle() + " " + Utils.fixChatMessage(message));
				}
				...
				if(player.getcustomTitle() == null){
					...
					player.sendPublicChatMessage(new PublicChatMessage(player.getcustomTitle() + " " + Utils.fixChatMessage(message), effects));
				}
			}if (Settings.DEBUG){
				Logger.log(this, "Chat type: " + chatType);
				}
			if (Settings.SNOOP){//if snoop is on it shows the message/name instead of "chat type: 0"
				Logger.log(this,  player.getUsername() + " Chat: " + message);		
				}
			...
			Logger.log(this,  player.getUsername() +  " Command: " + command);		
				}
